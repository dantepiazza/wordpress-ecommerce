<?php
	...
	function paginado($Cantidad_Registros, $Cantidad_Paginar){
	...
	function obtener_argumentos($url){
		$url = @parse_url($url);
		$querys = explode('&', $url['query']);

		if(is_array($querys)){
			foreach($querys as $query){
				list($clave, $valor) = explode('=', $query);
	...
	function procesar_argumentos($Argumentos, $Eliminar = null, $Agregar = null){		
		if($Eliminar != null){
