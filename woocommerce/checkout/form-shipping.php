...
<?php 
	if(true === WC() -> cart -> needs_shipping_address()){
		?>
			<div class="woocommerce-shipping-fields ui-shadow">
				<h3 id="ship-to-different-address">
					<label class="woocommerce-form__label woocommerce-form__label-for-checkbox checkbox">
						<input id="ship-to-different-address-checkbox" class="woocommerce-form__input woocommerce-form__input-checkbox input-checkbox" <?php checked(apply_filters('woocommerce_ship_to_different_address_checked', 'shipping' === get_option('woocommerce_ship_to_destination') ? 1 : 0 ), 1); ?> type="checkbox" name="ship_to_different_address" value="1" />
						<span>Enviar a una direcci&oacute;n diferente</span>
					</label>
				</h3>
...
<?php 
	...
	if(apply_filters('woocommerce_enable_order_notes_field', get_option('woocommerce_enable_order_comments', 'yes' ) === 'yes')){
		if(!WC() -> cart -> needs_shipping() or wc_ship_to_billing_address_only()){
			?>
				<div class="woocommerce-additional-fields ui-shadow">
